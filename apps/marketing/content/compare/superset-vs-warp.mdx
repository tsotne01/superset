...
Superset is a desktop app for orchestrating AI coding agents. You spin up 10+ agents on separate tasks simultaneously, each in its own Git worktree with a dedicated branch and working directory. When one finishes, you review the diff and merge. Superset ships no AI model — you bring Claude Code, Codex, Aider, or any CLI agent. It handles parallel execution, worktree isolation, terminal persistence, and code review. Source-available under Elastic License 2.0 (ELv2) and built around local Git worktrees.
