...
Superset is a desktop terminal that orchestrates CLI-based coding agents in parallel across isolated Git worktrees. Each agent — Codex, Claude Code, Aider, OpenCode, or any CLI process — gets its own branch and working directory. A persistent daemon manages sessions so they survive app restarts and crashes. Includes a built-in diff viewer and integrates with VS Code, Cursor, JetBrains, and Xcode. Source-available under Elastic License 2.0 (ELv2).
...
Cloud Codex runs in OpenAI's VMs and produces pull requests. Superset runs agents locally in Git worktrees. Cloud Codex offloads compute but sends code to OpenAI's servers. Superset keeps orchestration on your machine and lets you choose which agents and providers to use. They serve different trust and infrastructure models.
